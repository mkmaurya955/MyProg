...
public class JwtUtils {
	...
	public static String createJWTToken(String username, String secret, long expiration, List<String> grantedAuthorities) {
		...
		return Jwts.builder()
				...
				.signWith(HS256, new SecretKeySpec(secret.getBytes(StandardCharsets.UTF_8), ALG_NAME))
	...
	public static Optional<String> getTokenWithoutBearer(Optional<String> tokenWithBearerPrefix) {
		return tokenWithBearerPrefix.map(s -> org.apache.commons.lang3.StringUtils.substring(s, BEARER_INDEX));
	}
	...
	public static Claims extractAllClaims(String token, String secret) {
		return Jwts
				.parser()
				.setSigningKey(new SecretKeySpec(secret.getBytes(StandardCharsets.UTF_8), ALG_NAME))
