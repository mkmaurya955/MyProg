...
import com.example.ldap.entity.PortalUser;
...
public class PortalUserService implements UserDetailsService {
	...
	private void prepareLdapContext() {
		String ldapFullUrl = new StringBuilder(this.ldapUrl).append(":").append(this.ldapPort).append("/")
				.append(this.ldapRoot).toString();
	...
	/**
	 * Locates the user from remote LDAP and returns the information wrapped in a
	 * {@link PortalUserPrincipal} object.
	 *
	 * @param username: Username to be queried from LDAP. Mostly, case-insensitive.
	 * @return {@link UserDetails}
	 * @throws {@link UsernameNotFoundException} when there is no entry matches with
	 *                <code>username</code>.
	 * @throws {@link IncorrectResultSizeDataAccessException} when there is more
	 *                than one entry matches with <code>username</code>.
	 */
	@Override
	public UserDetails loadUserByUsername(String username) {
		try {
			...
			searchControls.setReturningAttributes(
					new String[] { Constants.LDAP_ATTRIBUTE_ISMEMBEROF, Constants.LDAP_ATTRIBUTE_UID });
			SpringSecurityLdapTemplate template = new SpringSecurityLdapTemplate(this.contextSource);
			template.setSearchControls(searchControls);

			DirContextOperations searchResult = template.searchForSingleEntry(this.ldapUserSearchBase,
					this.ldapUserSearchFilter, new String[] { username });
			...
			return new PortalUserPrincipal(
					PortalUser.builder().username(username).grantedAuthorities(grantedAuthorities).build());
	...
	/**
	 * Authenticates the user from remote LDAP by using <code>username</code> and
	 * <code>password</code> credentials provided. Populates the
	 * {@link SecurityContextHolder} with an {@link Authentication} object.
	 *
	 * @param username
	 * @param password
	 *
	 * @return An {@link AuthResponse} object populated with user detail information
	 *         and a JWT token to be used in following calls.
	 */
	public AuthResponse authenticateUser(String username, String password) {
		...
		PortalUserPrincipal portalUserPrincipal = new PortalUserPrincipal(
				PortalUser.builder().username(username).grantedAuthorities(grantedAuthorities).build());
		...
		List<String> userRoles = portalUserPrincipal.getAuthorities().stream().map(GrantedAuthority::getAuthority)
				.collect(Collectors.toList());

		return AuthResponse.builder().username(username).message(Constants.MESSAGE_SUCCESS)
				.status(Constants.STATUS_CODE_SUCCESS).userRoles(userRoles)
				.token(JwtUtils.createJWTToken(username, this.jwtSecret, this.jwtTimeout, userRoles)).build();
	}

	private List<String> doLdapSearch(String username, String password) {
		try {
			PortalUserPrincipal portalUserPrincipal = new PortalUserPrincipal(
					PortalUser.builder().username(username).build());
			Authentication authentication = new UsernamePasswordAuthenticationToken(portalUserPrincipal, password);
			PasswordComparisonAuthenticator passwordComparisonAuthenticator = new PasswordComparisonAuthenticator(
					this.contextSource);
			passwordComparisonAuthenticator.setPasswordEncoder(new LdapShaPasswordEncoder());
			passwordComparisonAuthenticator
					.setUserDnPatterns(new String[] { this.ldapUserSearchFilter + "," + ldapUserSearchBase });
			passwordComparisonAuthenticator.setUserAttributes(
					new String[] { Constants.LDAP_ATTRIBUTE_ISMEMBEROF, LDAP_ATTRIBUTE_USERPASSWORD });
	...
	private List<String> getGrantedAuthorities(DirContextOperations ldapResult) {
		if (ArrayUtils.isEmpty(ldapResult.getStringAttributes(Constants.LDAP_ATTRIBUTE_ISMEMBEROF))) {
			log.info("No roles found for user: {}. Returning empty granted authorities list.",
					ldapResult.getStringAttribute(Constants.LDAP_ATTRIBUTE_UID));
